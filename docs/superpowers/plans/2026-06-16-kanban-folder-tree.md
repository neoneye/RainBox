...
**Goal:** Give the `/kanban` left panel the same nested folder-tree `/chat` and `/cron` have (folders → boards), as a full port of the pattern in `notes/ui-left-panel-tree.md`.
...
**Files:**
- Modify: `notes/ui-left-panel-tree.md` (mark `/kanban` as a third implementation)
...
In `notes/ui-left-panel-tree.md`, update the intro so `/kanban` is listed as a built reference rather than a hypothetical. Replace:
...
```bash
git add notes/ui-left-panel-tree.md
