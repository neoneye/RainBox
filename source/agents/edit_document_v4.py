"""EditDocumentAgentV4 — replace_lines patch planner.
...
EDIT_DOCUMENT_V4_SYSTEM_PROMPT: str = """\
...
instruction describing a change. Produce a list of `replace_lines`
patches that, applied bottom-to-top, realize the change.
...
Output:
{"patches":[{"start_line":1,"end_line":1,"replacement":"# DONE: write greeting","intent":"Mark TODO complete."}],"status":"done","comment":"Marked the TODO on line 1 as DONE."}
...
Output:
{"patches":[{"start_line":3,"end_line":3,"replacement":"","intent":"Remove duplicate beta."}],"status":"done","comment":"Deleted the duplicate beta on line 3."}
...
Output:
{"patches":[{"start_line":4,"end_line":3,"replacement":"dates","intent":"Append 'dates' after line 3."}],"status":"done","comment":"Appended 'dates' after the last line."}
"""
...
class ReplaceLinesPatch(BaseModel):
    ...
    replacement: str = Field(
        description=(
            ...
            'start_line) may supply non-empty text or "" for a blank line.'
        )
    )
...
class EditPlan(BaseModel):
    patches: list[ReplaceLinesPatch] = Field(
        description=(
            "List of non-overlapping replace_lines patches applied "
            "bottom-to-top to realize the requested change. May be empty "
            "when status is 'unclear' or for a 'done' no-op."
        )
    )
...
def validate_patches(patches: list[ReplaceLinesPatch], document_line_count: int) -> None:
...
class EditDocumentAgentV4(StructuredLLMAgent):
    ...
    Journal result shape:
        {
            ...
            "comment": "<non-empty>",
            "patches": [<ReplaceLinesPatch.model_dump()>, ...],
        }
    ...
    def __init__(self, agent_uuid: UUID, name: str, send: StatusSender) -> None:
        super().__init__(
            ...
            name,
            send,
            system_prompt=EDIT_DOCUMENT_V4_SYSTEM_PROMPT,
    ...
    def _extract_payload(self, payload: dict[str, Any]) -> tuple[str, str]:
        ...
        if not isinstance(document, str):
            raise ValueError(
                "edit_document_v4 payload missing 'document' (use \"\" to "
                "generate content from scratch)"
            )
        if not isinstance(instructions, str) or not instructions.strip():
            raise ValueError(
                "edit_document_v4 payload missing or blank 'instructions'"
            )
