...
CAPABILITIES: dict[AssistantActionName, Capability] = {
    AssistantActionName.REPLY: Capability(
        ...
        description=('give your final answer to the user; ends the turn. '
                     ...
                     'sub-question? — then against acceptance_criteria_json, '
                     ...
                     'separators, or the wrong language or language '
                     'variant. The audit is a bare verdict, never a '
...
class AssistantAgent(ModelGroupAgent):
    ...
    def _run_acceptance_criteria_calls(
        ...
        *,
        ...
        if not self.candidate_model_uuids:
            ...
            # no rows, no section, the run proceeds as without the step.
    ...
    def _validate_decision(self, decision: AssistantStepDecision) -> str | None:
            ...
            if not isinstance(value, str) or not value.strip():
                hints = {
                    ...
                    "2_audit": (
                        ...
                        '"OK" if it answers the whole current message and '
                        "complies with acceptance_criteria_json and the "
                        "user settings, otherwise what is wrong"
                    ),
                }
                hint = hints.get(key, "")
                return (
        ...
        # after the message exists — otherwise the audit is a reflex "OK",
        # not a re-read. Checked in BOTH representations: the parsed dict
        # preserves json insertion order when the parser is faithful, and
        # the raw response text is the authority when it normalizes (a live
        # reversed reply slipped through the raw check alone — the dict
        # check has no plumbing to depend on).
    ...
    AUDIT_ORDER_ERROR: str = (
        ...
        '"1_message" (the full answer), then "2_audit" (a re-read of the '
