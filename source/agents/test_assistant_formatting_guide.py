...
def test_reversed_args_dict_is_rejected_without_raw_text(room):
    """The live miss: a fully reversed reply (2_audit before 1_message)
    shipped when only the raw-text check guarded order. The parsed dict
    preserves json insertion order, so the dict-side check catches it with
    no dependency on the raw-text plumbing."""
...
def test_out_of_prefix_order_raw_response_is_rejected(room):
    ...
    the model retries writing message, then audit."""
    agent = AssistantAgent(agent_uuid=ASSISTANT_UUID, name="assistant",
                           send=lambda _: None)
    prompts = []
    script = [
...
def test_system_prompt_documents_the_reply_args(room):
    ...
    # The message obeys the run-level criteria; the audit re-reads against
    # the current message and the criteria — language rules live in the
    # criteria machinery now, not the reply description.
    assert "obeying acceptance_criteria_json" in system
    assert "every sentence and sub-question" in system
