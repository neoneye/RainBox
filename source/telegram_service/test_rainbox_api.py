...
def test_post_message_returns_created_row():
    session = FakeSession([FakeResponse({"id": 9, "uuid": "m9"}, status=201)])
    client = RainboxClient("http://127.0.0.1:5000/", session=session)
...
def test_get_messages_after_passes_cursor():
    session = FakeSession([FakeResponse([{"id": 10}])])
    client = RainboxClient("http://127.0.0.1:5000/", session=session)
