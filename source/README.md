...
Install the **pgvector** Postgres extension (used for vector similarity over the seed Q&A knowledge base and the memory claims):
...
**Local model providers.** Ollama is the preferred default. Run it on `127.0.0.1:11434`; Jan on `127.0.0.1:1337` and LM Studio on `127.0.0.1:1234` remain supported alternatives. Also run Ollama with the embedding model used by Q&A and memory embeddings:

- one or more chat-capable models in a provider that the bound agents' model groups will reference on `/agentmodel`, and
- the Ollama embeddings model **`embeddinggemma:300m`** (768-dim) — seed Q&A and memory embeddings use Ollama's OpenAI-compatible `/v1/embeddings` endpoint to embed Q&A, active memory claims, and incoming queries.
...
- `http://127.0.0.1:5000/` — index: the agent roster, each linking to a page that can enqueue an inbox item by hand. The supervisor's next tick (within ~5 s when idle — it backs off its poll when there's no work — then ~1 s while an agent is active) spawns whichever agent has work.
...
- `agents/query_handlers.py` — the seed KB's **dynamic** handlers (`HANDLERS`) plus the `QueryContext` they run with. A seed entry of kind `dynamic` names a handler instead of carrying a static answer, and the handler produces the answer at retrieval time (project status, git status, capabilities, model info).
- `agents/recall_filter.py` — the relevance filter over recalled candidates: the `FilterScore`/`FilterDecision` schema the scorer answers in, the standalone `FILTER_SYSTEM_PROMPT`, the prompt-row builders, and `apply_filter_scores` — the keep/drop policy, decided in code from the scorer's numbers rather than by the prompt. `agents/recall_reranker.py` is the cross-encoder backend that fills the same `ScoredCandidate` rows in without an LLM.
- `agents/model_groups.py` — which model group a call runs on and how the call is made: `resolve_model_group` walks a binding chain, `resolve_assistant_model_group` is the assistant's two-link `<slot> → assistant.default` version of it, and `structured_llm_call` makes one structured-output call that falls back through the group's members. Used by callers that are not agents at all (the assistant's own calls, /memory/developer, the eval harnesses).
...
- `llm/` — provider-backed LlamaIndex connectivity: native Ollama model construction plus Jan and LM Studio support; a `ThinkingAwareOpenAILike` subclass that recovers JSON from `reasoning_content` for OpenAI-compatible providers; and the `/model` test probes — `test_chat` (plain completion), `test_structured_output` (`PingResponse` schema), and `test_tool_call` (`FunctionAgent` + `send_number` tool).
...
4. Pick the agent class for the role — `resolve_agent_class` looks the role up in `AGENT_CLASS_PATHS` and imports only that one module, so a spawned agent does not load every other agent's dependencies; a role absent from the table falls back to `ModelGroupAgent` — and call `agent_instance.run()`.
...
- `StructuredLLMAgent.handle` — builds two messages (a fixed system prompt + a per-item `user_prompt(payload)`), tries each model in the group in priority order via `as_structured_llm(...)`, and returns the parsed Pydantic model. `StructuredChatAgent` and the `edit_document*` agents build on this.
...
```
{"status": "processing", "journal_id": 7, "payload": {"task": "..."}}
...
1. **A role with no class is a stub, not an error.** `StructuredChatAgent`, `UnstructuredChatAgent`, `RouterAgent`, the `edit_document*` agents, `MCPAgent` and the assistant make real provider-backed LLM calls when configured. A role absent from `AGENT_CLASS_PATHS` falls back to `ModelGroupAgent`, whose `handle()` resolves its real model-group candidates and then sleeps a second — deliberately functional, so binding can be verified without an LLM, but silent about the fact that nothing is actually running.
...
This behaves like a multi-agent system: durable, type-checked, observable through Flask-Admin, with end-to-end lineage carried in each routed payload, real structured LLM calls behind the chat agents and the assistant's ReAct loop, a live group-chat UI over SSE, and a single Ctrl-C-stoppable entrypoint. What's left to make it useful for broader LLM workloads is semantic-progress detection beyond liveness heartbeats, a retry/backoff policy for `failed` rows, and a routing topology richer than one successor.
...
Explicit memory commands are handled by `memory/ops.py`, matched before the Q&A/vector path initializes:
