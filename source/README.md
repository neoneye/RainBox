...
Nineteen roles are defined. Three — `dreamer`, `critic`, `verifier` — form a linear pipeline: each agent's `completed` journal row is routed by the supervisor into the next agent's inbox. Clicking **Run demo** in the webapp seeds 5 items into dreamer's inbox; the supervisor wakes dreamer, which produces 5 critic items, which produce 5 verifier items, with full lineage carried in each payload. Ten roles — `followup`, `chat_structured`, `chat_unstructured`, `tool_demo`, `workspace_shell`, `router`, `query`, `query_router`, `query_filter_router`, and `mcp` — back the group-chat feature; `workspace_shell` (deterministic command runner) and `query` (embedding-only retriever) skip the LLM completion path, while the others make **real LLM calls** when configured with model groups. Six `edit_document*` roles are payload-driven document-editing planners: they consume `{document, instructions}` inbox payloads, return validated patch data in the journal result, and progressively experiment with richer patch schemas, status/comment fields, EOF handling, and reasoning fields. They plan edits but do not apply them directly.

Agents are a small **class hierarchy** (`agent.py`): a base `Agent` owns the inbox-drain lifecycle, `ModelGroupAgent` resolves the agent's bound model group, and `StructuredLLMAgent` makes one schema-validated LLM call per item. The `dreamer/critic/verifier` pipeline agents still run a `time.sleep(1)` placeholder, but the chat-backing agents make real calls: `FollowUpClassifierAgent` (`agent_followup.py`) classifies whether a chat message needs a reply, and `StructuredChatAgent` (`agent_chat_structured.py`, role `chat_structured`) reads a chatroom's history, retrieves relevant long-term memories, and posts a reply — both `StructuredLLMAgent` subclasses. Its plain-text sibling `UnstructuredChatAgent` (`agent_chat_unstructured.py`, role `chat_unstructured`) does the same history read and memory retrieval but makes a single *non-structured* completion, so it subclasses `ModelGroupAgent` directly and needs a model group declared "structured output: must not have". `ToolDemoAgent` (`agent_tool_demo.py`) is a sibling that subclasses `ModelGroupAgent` directly and replies via a LlamaIndex `FunctionAgent` equipped with a `multiply` tool, demonstrating the **tool-calling** path instead of structured output. `WorkspaceShellChatAgent` (the `tools/` package, role `workspace_shell`) is a **no-LLM** sibling that subclasses the base `Agent` directly: it parses a chatroom's typed command into an explicit argv with `shlex` and runs it with `subprocess.run(shell=False)` — no shell interpretation at all — then posts the raw output back. Each command has its own validator and every path argument is confined to a workspace directory. It is split into focused modules (`workspace_policy` / `command_policy` / `workspace_command_runner` / `workspace_shell_chat`). `RouterAgent` (`router_agent.py`, role `router`) is another `StructuredLLMAgent` (no FunctionAgent): it reads the same IRC-style transcript and triages the latest message into `{subject, action, reply}` — a 10-20 word summary, whether it needs an action (`no`/`unclear`/`yes`), and a clarification or small-talk reply — posting the reply as a real message and the triage as a `debug-router` row. `QueryAgent` (`query_agent.py`, role `query`) is the no-LLM-completion retriever: it embeds the message via LM Studio's nomic embedding endpoint, looks the user up against a pgvector-backed Q&A registry (`memory/question_answer.jsonl`), and either posts the matched static `answer` or invokes the named dynamic handler in `query_handlers.HANDLERS`. `QueryRouterAgent` (`query_router_agent.py`, role `query_router`) is the crossover: exact alias hits bypass the LLM entirely, and otherwise the top-1 ungated semantic candidate (with handler output materialized) is fed as a hint to a router-style LLM. `QueryFilterRouterAgent` (`query_filter_router_agent.py`, role `query_filter_router`) splits that path further: one LLM call filters top-K candidates for relevance, then a shorter routing LLM produces the reply using only the kept candidates. `MCPAgent` (`agent_mcp.py`, role `mcp`) runs a FunctionAgent with tools loaded from configured MCP servers.

On top of the supervisor, the webapp carries a **local-LLM toolkit** for picking and tuning the model that backs the agents. It connects to [LM Studio](https://lmstudio.ai/) through LlamaIndex (`OpenAILike`), syncs the currently-loaded models into a `model_config` table on startup, and provides pages to ping a model (plain text and structured JSON output), define reusable parameter overrides per model, organize models into priority-ordered groups, bind each agent to one of those groups (the fallback list it should run), and run a benchmark suite that scores each model/override.

The webapp also hosts a **group chat** (`/chat`): one human operator and the agents converse in rooms. Rooms, messages, users, and membership are persisted in Postgres; new messages are pushed to the browser over **Server-Sent Events** (no polling) via Postgres `LISTEN/NOTIFY`. When the operator posts in a room a responder agent (`chat_structured`, `chat_unstructured`, `tool_demo`, `workspace_shell`, `router`, `query`, `query_router`, `query_filter_router`, or `mcp`) belongs to, a job is enqueued and the agent replies live (rendered as markdown, or as a JSON code block when it answers with JSON). Agent replies can be upvoted/downvoted; that feedback is stored, can be promoted into eval cases, and can be linked back to retrieval telemetry. See [As a loop for running AI agents](#as-a-loop-for-running-ai-agents) for an assessment of what's real and what's still placeholder.
...
**LM Studio models.** Make sure LM Studio is running on `127.0.0.1:1234` and has at least:

- a chat-capable model (any that the bound agents' model groups will reference on `/agent_models`), and
- the embeddings model **`text-embedding-nomic-embed-text-v1.5`** (768-dim) — `QueryAgent` calls the OpenAI-compatible `/v1/embeddings` endpoint with this model to embed the JSONL knowledge base and incoming queries.
...
- `http://127.0.0.1:5000/agent_models` — bind each code-defined agent to a model group (the prioritized fallback list it should run); editable without code changes. Agents that need tool calls (those with `requires_function_calling` in `agent_config.py`, e.g. `tool_demo` and `mcp`) are only offered function-calling groups, so they can't be misconfigured onto a structured-output-only group.
...
- `http://127.0.0.1:5000/benchmark_editdocument` — per-model scoreboard for `EditDocumentAgent` and `EditDocumentAgentV2`. Picker switches between the two agents. Columns are the seeded tests in `benchmark_editdocument.py` (currently `append_task`, `remove_task`, `check_task`); rows are every available model in the `/models` tree. Each cell runs the agent end-to-end with the target model pinned (no fallback) and scores by byte-for-byte equality between the applied patches and a reference expected document. Cell details reveal expected / applied / patches and (for v2) status / comment.
...
To see the `chat_structured` agent reply, assign it a model group at `/agent_models`, keep LM Studio running, and post a message in a room it's a member of (`main.py` must be running so the supervisor can spawn it).
...
- `db.py` — Flask-SQLAlchemy models, queue + model-management + chat API, app factory, memory/provenance helpers, feedback helpers, retrieval telemetry helpers, and eval case/run/result helpers.
- `llm.py` — LM Studio connectivity via LlamaIndex: model listing, `fetch_native_models()` (the native `/api/v0/models` REST API, which exposes per-model `capabilities`/metadata), a `ThinkingAwareOpenAILike` subclass that recovers JSON from `reasoning_content`, and the three `/models` test probes — `test_chat` (plain completion), `test_structured_output` (`PingResponse` schema), and `test_tool_call` (`FunctionAgent` + `send_number` tool).
...
- `benchmark_runner.py` — background orchestrator that walks the model tree, warms up each model once, runs every benchmark, and maintains the live state dict the `/benchmark_basic` page polls. The kanban 2×2 matrix (`benchmark_kanban.py`) runs as its own spec set on `/benchmark_kanban`.
- `benchmark_editdocument.py` / `benchmark_editdocument_runner.py` — second benchmark stack, scoped to document-editing agents. Mirrors `benchmark.py`/`benchmark_runner.py` shape. The naming prefix `benchmark_<agentname>_*` is the convention for future agent benchmarks.
- `patch_apply.py` — `apply_patches(document, patches) -> str`. Materializes a list of `replace_lines` patches (the encoding `validate_patches` accepts) into the post-edit document. Standalone helper used by the benchmark.

`main.py` launches `agent.py` via `posix_spawn`; the agent's socket fd is inherited across the exec. Each process pushes a Flask app context to get `db.session` access; each agent only touches journal rows tagged with its own `uuid`, so there is no cross-agent contention.
...
- `webapp/core.py` — builds `app`, runs `init_db`, syncs `model_config` from LM Studio on startup, creates `benchmark_runner`, and registers the Flask-Admin views. Imported first; everything else registers routes against its `app`.
...
3. `os.posix_spawn` runs `agent.py` with `--socket-fd <agent_end_fd>`.
...
1. **Routing pass:** scan the journal for rows in `state='completed' AND routed_at IS NULL`. For each, look up the source agent's `next` uuid in `agent_config`. If non-null, enqueue a lineage payload into that next agent's inbox. Mark the row routed regardless (terminal rows get marked too, so they aren't re-scanned).
2. **Wake-up pass:** call `db.agent_uuids_with_work()` (returns the set of agent UUIDs whose inbox is non-empty). For each role in `agent_config` whose UUID is in that set and whose agent is not currently running, `spawn()` it and register its socket on the `selectors`.
...
The supervisor's loop includes a routing pass that turns one agent's `completed` journal row into the next agent's inbox row. Topology lives in `agent_config` as a `next: UUID | None` field per role:
...
`agent_model_binding` links a code-defined agent (its `uuid` from `agent_config.py`) to the `model_group` it should run — the prioritized fallback list, e.g. a fast/low-quality vs a slow/high-quality strategy. Agents and their pipeline topology stay in code; only this model-to-agent assignment lives in the database, editable at runtime via `/agent_models`. `init_db` seeds one unassigned row per code agent; `model_group_uuid` is nullable, with `ON DELETE SET NULL` so deleting a group just unassigns the agents that used it. `model_group.requires_function_calling` is a membership constraint: when set, only function-calling models/overrides may join, and `/agent_models` only offers such groups to agents that declare `requires_function_calling`.
...
`feedback_event` captures up/down feedback on user-facing agent replies and snapshots enough same-turn context to promote the feedback into an eval case later. `retrieval_event` is an event log for Q&A and memory retrieval decisions (`retrieved`, `accepted`, `rejected`, `used`, `downvoted`); counters are derived from it, not stored as mutable fields. `eval_case` / `eval_run` / `eval_result` are the deterministic benchmark loop used by `eval_runner.py`, `eval_compare.py`, and `eval_optimizer.py`.
...
Helpers in `db.py` (all use `db.session` under a pushed Flask app context — no connection argument):
...
- `main.py` — single entrypoint: webserver + supervisor thread + signal handlers (plus the `--force-lmstudio-model-sync` one-shot).
- `webapp/` — Flask app package: `core` (app + admin + startup sync), `pages`, `models_views`, `model_group_views`, `agent_views`, `benchmark_views`, `chat_views`, `chat_api`, `cron_views`/`cron_api` (the `/cron` scheduler UI), and `settings_views` (the `/settings` config editor).
- `agent.py` — agent process + class hierarchy (`Agent` → `ModelGroupAgent` → `StructuredLLMAgent`); reads config, drains inbox via `take_item`, journals state transitions.
- `agent_config.py` — the eighteen role declarations + pipeline topology.
- `agent_followup.py` — `FollowUpClassifierAgent` (does a chat message need a reply?).
- `agent_chat_structured.py` — `StructuredChatAgent` (memory context + IRC-style transcript → reply posted back to the room).
- `agent_chat_unstructured.py` — `UnstructuredChatAgent` (same inputs as `StructuredChatAgent`, but a single plain-text completion instead of structured output).
- `router_agent.py` — `RouterAgent` (IRC-style transcript → structured `{subject, action}` triage → JSON decision posted back to the room; no FunctionAgent).
- `query_agent.py` / `query_handlers.py` — `QueryAgent` (pgvector retriever over `memory/question_answer.jsonl`; static answer or dynamic handler).
- `query_router_agent.py` — `QueryRouterAgent` (exact alias → no LLM; otherwise router LLM with the top semantic candidate as a hint).
- `query_filter_router_agent.py` — `QueryFilterRouterAgent` (two-stage LLM: filter top-K for relevance, then route over the kept candidates with a simpler prompt).
- `agent_tool_demo.py` — `ToolDemoAgent` (IRC-style transcript → `FunctionAgent` with a `multiply` tool → reply posted back to the room).
- `agent_edit_document.py` — `EditDocumentAgent` ({document, instructions} → validated `replace_lines` patches; planner only, does not apply).
- `agent_edit_document_v2.py` — `EditDocumentAgentV2` (same payload as v1; result includes `status` + non-empty `comment` alongside `patches`).
- `agent_edit_document_v3.py` — `EditDocumentAgentV3` (4-op high-level patch language normalized to replace_lines; EOF marker in the prompt).
- `agent_edit_document_v4.py`, `agent_edit_document_v5.py`, `agent_edit_document_v6.py` — later document-editing variants for logical EOF handling, experimental baselines, and reasoning-before-patches.
- `agent_mcp.py` — `MCPAgent` (chat-backed FunctionAgent using tools from `mcp.json` + the customize.dir overlay).
- `tools/` — `WorkspaceShellChatAgent` (role `workspace_shell`), a no-LLM, no-`bash` command runner split into a small package: `workspace_policy.py` (path confinement + sensitive-file blocking), `command_policy.py` (parsing + allowlist + per-command validators), `workspace_command_runner.py` (`subprocess.run(shell=False)` + `cd`/`pwd` builtins), `workspace_shell_chat.py` (the agent that composes them). Tests live alongside (`tools/test_*.py`, shared fixtures in `tools/conftest.py`).
- `db.py` — Flask-SQLAlchemy models, queue + model-management + chat API, app factory, memory/provenance helpers, feedback helpers, retrieval telemetry helpers, and eval case/run/result helpers.
- `db_settings.py` — operator-configuration registry + accessors (`get_setting`/`set_setting`/`all_settings`) over the `app_setting` table; resolves DB → env → default. See `docs/proposals/2026-06-07-user-configuration-in-postgres.md`.
- `backup_db.py` — database backup: `pg_dump | zstd | age -r <pubkey>` to a dated, public-key-encrypted `.zstd.age` file (CLI is flags/env only). See `docs/backup.md`.
- `backup_remote.py` — optional off-machine upload: commit the encrypted backup into the backup-repo git repo and push.
- `llm.py` — LM Studio / LlamaIndex connectivity, native-API model metadata, and the chat / structured-output / tool-calling test helpers.
- `benchmark.py`, `benchmark_runner.py` — benchmark definitions and the background runner behind `/benchmark_basic`.
- `benchmark_editdocument.py` / `benchmark_editdocument_runner.py` — benchmark stack for `EditDocumentAgent` / V2.
- `patch_apply.py` — `apply_patches` helper.
- `memory_ops.py`, `memory_retrieval.py` — explicit memory command parsing/handling, deterministic memory retrieval, memory context formatting, and `debug-memory` audit rows.
- `eval_runner.py`, `eval_compare.py`, `eval_optimizer.py`, `eval_monitor.py` — deterministic eval runner, baseline comparison/regression gate, bounded candidate optimizer, and production-sample monitor.
- `docs/memory-architecture.md`, `docs/relevance-telemetry.md` — current architecture notes for memory provenance, retrieval, feedback, evals, and telemetry.
- `test_agent_chat_memory.py`, `test_agent_chat_unstructured.py`, `test_agent_followup.py`, `tools/test_*.py` — pytest tests (`python -m pytest`); the followup/chat LLM tests skip unless a model group is assigned and LM Studio is reachable, but the `workspace_shell` tests are fully deterministic (real subprocess + Postgres, no LM Studio, no skips).
...
1. **LLM is wired for the chat/tool/document agents, not the demo pipeline.** `StructuredChatAgent`, `UnstructuredChatAgent`, `FollowUpClassifierAgent`, router variants, query-router variants, document-editing agents, and tool agents make real LM Studio calls when configured. The `dreamer/critic/verifier` pipeline agents still run the `time.sleep(1)` placeholder — swapping them over is now a matter of giving each a `StructuredLLMAgent` subclass with its own system prompt, not new plumbing.
...
- **Path A — Put the pipeline on the LLM.** The plumbing now exists: `StructuredLLMAgent` makes a real structured LM Studio call, and `StructuredChatAgent`/`FollowUpClassifierAgent` already use it. Give `dreamer`, `critic`, and `verifier` their own `StructuredLLMAgent` subclasses with role-specific system prompts (dreamer generates ideas, critic critiques, verifier verifies) and register them in `agent.py`'s class registry. The infrastructure (model-group binding, fallback, journaling, the raised heartbeat) is already in place, so this is mostly prompt design now.
...
Explicit memory commands are handled by `memory_ops.py` and routed through `QueryAgent` before the Q&A/vector path initializes:
...
- `eval_runner.py` runs deterministic `chat_reply` and `memory_retrieval` cases.
- `eval_compare.py` compares candidate runs against baselines and gates regressions.
- `eval_optimizer.py` tries bounded candidate configs such as `memory_retrieval_limit`.
- `eval_monitor.py` samples recent production chat outputs into eval runs.
