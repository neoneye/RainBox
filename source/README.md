...
- `http://127.0.0.1:5000/benchmark_editdocument` — per-model scoreboard for `EditDocumentAgent` and `EditDocumentAgentV2`. Picker switches between the two agents. Columns are the seeded tests in `benchmarks/editdocument.py` (currently `append_task`, `remove_task`, `check_task`); rows are every available model in the `/models` tree. Each cell runs the agent end-to-end with the target model pinned (no fallback) and scores by byte-for-byte equality between the applied patches and a reference expected document. Cell details reveal expected / applied / patches and (for v2) status / comment.
...
- `agents/config.py` — the eighteen role declarations + pipeline topology.
...
- `webapp/core.py` — builds `app`, runs `init_db`, syncs `model_config` from LM Studio on startup, creates the benchmark runner, and registers the Flask-Admin views. Imported first; everything else registers routes against its `app`.
