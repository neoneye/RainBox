...
| `rainbox` | Claim, evidence, embedding, retrieval event | Postgres/SQLAlchemy plus pgvector | Hard-filtered hybrid: vector, Postgres full-text, entity boost; profile digest | User commands, assistant actions, review UI; active/candidate flows | Reject, supersede, reactivate, expiry, sensitivity; UI stale-write guards | Global, agent, room, project; sensitivity | Full assistant app: chat prompt, action loop, review UI | Embedding sync/prune, telemetry, feedback/eval loop | Strong claim/evidence provenance and retrieval audit | Operator governance, telemetry, eval integration | Compact claims may lose source nuance; active wrong claims can steer behavior |
...
- `rainbox`: reject claim, supersede claim, expire claim, prune embeddings; rejected/superseded evidence remains inspectable.
...
Update/delete APIs are not enough. A system needs to model contradiction, supersession, source, timestamp, and rejected values. Otherwise a wrong fact can be reintroduced by later extraction. Verel's rejected tombstones are the clearest countermeasure; RainBox's supersession lineage and retained rejected rows are a strong practical application pattern.
...
- Best idea: claim/evidence memory tied to review UI, retrieval telemetry, feedback, and eval gates.
- Biggest risk: active compact claims can steer behavior while losing nuance from original source context.
- Most reusable component: `MemoryClaim`/`MemoryEvidence`/`RetrievalEvent` model plus `retrieve_memories_hybrid()`.
- Maturity impression: strong app-integrated memory subsystem with broad tests and operator workflows.
- Study when: building an assistant product where memory must be inspectable and governable.
- Do not copy when: you need a small embeddable library or raw transcript recall as the primary memory layer.
