...
| `assistant.acceptance_criteria` | bool | `false` | — | Run the acceptance-criteria call before the assistant's decide loop (a code-driven step 0) and inject `<acceptance_criteria_json>` into every step: the reply's language, work-steering preferences, formatting, and disclosed assumptions, established before any work happens. Also enables the `acceptance_criteria` revision action. Default off until the step passes its eval gate. |
