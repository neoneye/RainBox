...
| `assistant.acceptance_criteria` | bool | `false` | — | Run the acceptance-criteria call before the assistant's decide loop (a code-driven step 0) and inject `<acceptance_criteria_json>` into every step: work-steering preferences, formatting, and disclosed assumptions, established before any work happens. Response language remains owned by the separate classifier. Also enables the `acceptance_criteria` revision action. Default off until the step passes its eval gate. |
