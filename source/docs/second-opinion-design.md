...
2. `<acceptance_criteria_json>` — this turn's established reply constraints
   (absent when the criteria call failed or was skipped; see
   `assistant-design.md` §Acceptance criteria). The criteria are part of
   what "serves the request" means: a program converting to yards should
   fail review when they say meters.
