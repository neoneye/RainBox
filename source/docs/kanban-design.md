...
| move to board | `POST …/move-to-board {boardId, columnId?, actor?, note?}` | move to another board (end of `columnId`, or the target's first column) keeping uuid, audit trail, assignee, and lease; the 'moved' event names both boards. The task modal's Board select uses this — the page's per-board bulk save cannot express a cross-board move without delete + recreate |
