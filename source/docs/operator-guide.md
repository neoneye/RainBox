...
- `/assistant` — assistant run inspector (traces, steps, write intents, and
  ...
  Every run starts with an `acceptance_criteria` step establishing the
  reply's constraints (language, target units, formatting, disclosed
  assumptions) before any work — inspect its `assumptions` there to spot a
  wrong guess at a glance; see `docs/assistant-design.md` §Acceptance
  criteria.
