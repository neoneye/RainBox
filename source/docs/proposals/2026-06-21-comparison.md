...
facts. RainBox should add a memory-provider seam and benchmark them before
outsourcing memory.
...
and Honcho are more specialized and likely stronger on large-scale retrieval,
cross-session benchmark performance, graph/reasoning, connectors, and packaged
SDKs. Honcho is the best conceptual complement for user/agent modeling.
