"""Unit tests for the lexical full-text seed ranking and the hybrid blend
(memory.seed_memory._fulltext_ranked / _hybrid_seed_ranked).

The motivating failure: "how is Robin related to the orienteering" — the token
`orienteering` appears verbatim in entry questions, but question-embedding
similarity ranked generic Robin entries above them, so the orienteering entries
never reached the filter. Full-text is the signal that catches exact content
words; the hybrid blend lets either signal surface a candidate.
"""

import pytest

from memory import seed_memory as qkb
from memory.seed_memory import Match, _fulltext_ranked, _hybrid_seed_ranked


@pytest.fixture
def kb(monkeypatch):
    """A small registry mirroring the real failure case: one entry with the
    rare content word in its questions, one with it only in the answer, and
    generic entries that share only the common token 'robin'."""
    entries = {
        "qa-orienteering": {
            "kind": "static", "path": "human.robin.orienteering",
            "questions": ["Orienteering / computer parties", "Robin's orienteering routes 1994-"],
            "answer": "Robin was active in the orienteering in the 1990s.",
        },
        "qa-sibling": {
            "kind": "static", "path": "human.sibling.identity",
            "questions": ["Who is Robin's brother?"],
            # The content word lives ONLY in the answer text.
            "answer": "Hans søskende er ikke navngivet her.",
        },
        "qa-name": {
            "kind": "static", "path": "human.robin.name",
            "questions": ["Who is Robin?"],
            "answer": "Robin is a software developer.",
        },
        "qa-food": {
            "kind": "static", "path": "human.robin.food",
            "questions": ["Does Robin have food allergy?"],
            "answer": "Robin has no food allergies.",
        },
        "qa-shielded": {
            "kind": "static", "path": "human.robin.secret", "shield": "locked.topic",
            "questions": ["Robin orienteering secret"],
            "answer": "Shielded orienteering entry.",
        },
    }
    monkeypatch.setattr(qkb, "_entries_by_id", entries)
    monkeypatch.setattr(qkb, "_unlocked_shields", lambda: set())
    monkeypatch.setattr(
        qkb, "_entry_locked", lambda entry, unlocked: bool(entry.get("shield")))
    return entries


def test_rare_content_word_outranks_common_token_matches(kb):
    ranked = _fulltext_ranked("how is Robin related to the orienteering")
    assert ranked[0].qa_id == "qa-orienteering"
    assert ranked[0].method == "fulltext"
    # Coverage, not max-normalization: 'related' matches nothing in the KB, so
    # even the best hit sits below 1.0 instead of a misleading full score.
    assert 0.0 < ranked[0].score < 1.0
    # Generic robin-only entries score strictly lower (IDF: 'robin' is common).
    by_id = {m.qa_id: m.score for m in ranked}
    assert by_id.get("qa-name", 0.0) < by_id["qa-orienteering"]


def test_full_question_coverage_scores_exactly_one(kb):
    """1.0 is reserved for a query whose every token is matched in the
    entry's questions — the only case that deserves to read like a full hit."""
    ranked = _fulltext_ranked("orienteering parties")
    assert ranked[0].qa_id == "qa-orienteering"
    assert ranked[0].score == pytest.approx(1.0)


def test_answer_only_token_is_found(kb):
    """A token that appears in no question but in an answer still surfaces the
    entry — the signal question embeddings can never see. Answer matches count
    at half a question match, so coverage reads 0.5, not 1.0."""
    ranked = _fulltext_ranked("søskende")
    assert [m.qa_id for m in ranked] == ["qa-sibling"]
    assert ranked[0].score == pytest.approx(0.5)


def test_question_match_outweighs_answer_match(kb):
    # 'orienteering' is in qa-orienteering's questions but only in qa-shielded's...
    # (shielded is excluded) — compare question-hit vs answer-hit weighting via
    # 'allergy': in qa-food's question AND answer vs nothing elsewhere.
    ranked = _fulltext_ranked("allergy")
    assert ranked[0].qa_id == "qa-food"


def test_shielded_entries_are_excluded(kb):
    ranked = _fulltext_ranked("orienteering secret")
    assert all(m.qa_id != "qa-shielded" for m in ranked)


def test_stopword_only_query_returns_nothing(kb):
    assert _fulltext_ranked("the and of") == []


def test_matched_question_is_the_best_overlapping_one(kb):
    ranked = _fulltext_ranked("orienteering parties")
    assert ranked[0].matched_question == "Orienteering / computer parties"


def test_entry_corpus_is_tokenized_once_across_queries(kb, monkeypatch):
    """The corpus side of the index (per-entry tokens + document frequencies)
    does not depend on the query, so a second query must tokenize only itself."""
    from memory import retrieval

    real_tokenize = retrieval._tokenize
    calls: list[str] = []

    def counting(text):
        calls.append(text)
        return real_tokenize(text)

    monkeypatch.setattr(retrieval, "_tokenize", counting)
    _fulltext_ranked("orienteering parties")
    assert len(calls) > 1, "first query must tokenize the entry corpus"
    calls.clear()
    _fulltext_ranked("food allergy")
    assert calls == ["food allergy"]


def test_index_rebuilds_when_the_registry_is_replaced(kb, monkeypatch):
    """rebuild_kb/sync_kb swap in a fresh registry; the cached index must not
    outlive it or every query answers from the old JSONL."""
    _fulltext_ranked("orienteering")
    monkeypatch.setattr(qkb, "_entries_by_id", {
        "qa-backup": {"kind": "static", "path": "system.backup",
                      "questions": ["Backup schedule"], "answer": ""},
    })
    assert [m.qa_id for m in _fulltext_ranked("backup")] == ["qa-backup"]


def test_index_rebuilds_when_a_shield_is_unlocked(monkeypatch):
    """Document frequencies are computed over the visible entries only, so the
    index is per unlocked-shield set — a Settings toggle takes effect on the
    next query, with no repopulate."""
    monkeypatch.setattr(qkb, "_entries_by_id", {
        "qa-open": {"kind": "static", "path": "system.backup",
                    "questions": ["Backup schedule"], "answer": ""},
        "qa-shielded": {"kind": "static", "path": "private.notes",
                        "shield": "locked.topic",
                        "questions": ["Private diary"], "answer": ""},
    })
    monkeypatch.setattr(qkb, "_unlocked_shields", lambda: set())
    assert _fulltext_ranked("diary") == []
    monkeypatch.setattr(qkb, "_unlocked_shields", lambda: {"locked.topic"})
    assert [m.qa_id for m in _fulltext_ranked("diary")] == ["qa-shielded"]


def test_hybrid_interleaves_both_signals(kb, monkeypatch):
    monkeypatch.setattr(qkb, "_semantic_ranked", lambda q, vs, **_: [
        Match(qa_id="qa-name", method="semantic", score=0.9, matched_question="Who is Robin?"),
        Match(qa_id="qa-orienteering", method="semantic", score=0.5,
              matched_question="Robin's orienteering routes 1994-"),
    ])
    ranked = _hybrid_seed_ranked("how is Robin related to the orienteering", None)
    # Interleave: best vector, then best full-text, deduplicated.
    assert [m.qa_id for m in ranked[:2]] == ["qa-name", "qa-orienteering"]
    by_id = {m.qa_id: m for m in ranked}
    assert by_id["qa-name"].method == "semantic+fulltext"
    assert by_id["qa-orienteering"].method == "semantic+fulltext"


def test_hybrid_fulltext_only_hits_reach_the_top_k_prefix(kb, monkeypatch):
    """The orienteering regression: entries absent from the embedding top-K got a
    capped blended score and were crowded out of any top-K slice by mediocre
    embedding matches. With rank interleaving, a strong full-text hit owns
    every second slot regardless of score scales."""
    monkeypatch.setattr(qkb, "_semantic_ranked", lambda q, vs, **_: [
        Match(qa_id="qa-name", method="semantic", score=0.75, matched_question="Who is Robin?"),
        Match(qa_id="qa-food", method="semantic", score=0.70,
              matched_question="Does Robin have food allergy?"),
        Match(qa_id="qa-sibling", method="semantic", score=0.65,
              matched_question="Who is Robin's brother?"),
    ])
    # qa-orienteering is NOT in the vector list; full-text ranks it #1.
    ranked = _hybrid_seed_ranked("how is Robin related to the orienteering", None)
    top3 = [m.qa_id for m in ranked[:3]]
    assert "qa-orienteering" in top3          # slot 2 belongs to full-text's best
    assert ranked[1].qa_id == "qa-orienteering"
    assert ranked[1].method == "fulltext"
    assert ranked[1].matched_question in (
        "Orienteering / computer parties", "Robin's orienteering routes 1994-")


def test_hybrid_budgets_cap_each_signal_independently(kb, monkeypatch):
    """"5 of this and 5 of that": each signal fills its own quota; neither is
    weighted over the other, and a budget of 0 disables its signal."""
    calls = []

    def fake_vec(q, vs, **_):
        calls.append("vec")
        return [Match(qa_id="qa-name", method="semantic", score=0.9,
                      matched_question="Who is Robin?"),
                Match(qa_id="qa-food", method="semantic", score=0.8,
                      matched_question="Does Robin have food allergy?")]

    monkeypatch.setattr(qkb, "_semantic_ranked", fake_vec)
    # 1 + 1: exactly the best of each signal.
    ranked = _hybrid_seed_ranked("how is Robin related to the orienteering", None,
                                 top_k_vector=1, top_k_fulltext=1)
    assert [m.qa_id for m in ranked] == ["qa-name", "qa-orienteering"]
    # 0 vector: the embedding ranker is never called; full-text only.
    calls.clear()
    ranked = _hybrid_seed_ranked("orienteering", None,
                                 top_k_vector=0, top_k_fulltext=2)
    assert calls == []
    assert ranked and all(m.method == "fulltext" for m in ranked)
    assert len(ranked) <= 2


def test_hybrid_degrades_to_fulltext_when_embeddings_fail(kb, monkeypatch):
    def boom(q, vs, **_):
        raise RuntimeError("embedding server down")

    monkeypatch.setattr(qkb, "_semantic_ranked", boom)
    ranked = _hybrid_seed_ranked("orienteering", None)
    assert ranked and ranked[0].qa_id == "qa-orienteering"
    assert ranked[0].method == "fulltext"
