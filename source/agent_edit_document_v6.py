"""EditDocumentAgentV6 — replace_lines patch planner.
...
EDIT_DOCUMENT_V6_SYSTEM_PROMPT: str = """\
...
Output exactly one JSON object: {"reasoning": ..., "patches": [...], "status": ..., "comment": ...}.
No prose, no markdown fences.

`reasoning` comes FIRST. Use 10-20 words to think out loud: name the
lines you intend to touch and which op you will use (replace or
insert). Always non-empty.
...
Output:
{"reasoning":"Line 1 is the TODO comment; rewrite it as DONE via a replace.","patches":[{"op":"replace","start_line":1,"end_line":1,"replacement":"# DONE: write greeting","intent":"Mark TODO complete."}],"status":"done","comment":"Marked the TODO on line 1 as DONE."}
...
Output:
{"reasoning":"Line 3 duplicates line 2's beta; delete line 3 with an empty replace.","patches":[{"op":"replace","start_line":3,"end_line":3,"replacement":"","intent":"Remove duplicate beta."}],"status":"done","comment":"Deleted the duplicate beta on line 3."}
...
Output:
{"reasoning":"Append 'dates' after the last line via insert with before_line=4.","patches":[{"op":"insert","before_line":4,"text":"dates","intent":"Append 'dates' after line 3."}],"status":"done","comment":"Appended 'dates' after the last line."}
...
Output:
{"reasoning":"Place 'bananas' between apples (line 1) and cherries (line 2) with insert before_line=2.","patches":[{"op":"insert","before_line":2,"text":"bananas","intent":"Insert 'bananas' before line 2."}],"status":"done","comment":"Inserted 'bananas' before cherries."}
"""
...
class EditPlan(BaseModel):
    reasoning: str = Field(
        min_length=1,
        description=(
            "Briefly think out loud before producing the patches: aim "
            "for 10-20 words. Name the lines you intend to touch and "
            "which op you will use (replace or insert). Always non-empty."
        ),
    )
...
class EditDocumentAgentV6(StructuredLLMAgent):
    ...
    def __init__(self, agent_uuid: UUID, name: str, send: StatusSender) -> None:
        super().__init__(
            ...
            name,
            send,
            system_prompt=EDIT_DOCUMENT_V6_SYSTEM_PROMPT,
    ...
    def _extract_payload(self, payload: dict[str, Any]) -> tuple[str, str]:
        ...
        if not isinstance(document, str):
            raise ValueError(
                "edit_document_v6 payload missing 'document' (use \"\" to "
                "generate content from scratch)"
            )
        if not isinstance(instructions, str) or not instructions.strip():
            raise ValueError(
                "edit_document_v6 payload missing or blank 'instructions'"
            )
