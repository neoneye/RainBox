"""Tests for agent_edit_document_v6.

v6 exposes two LLM-facing patch ops as a discriminated union (replace
...
document.
    ...
    python -m pytest test_agent_edit_document_v6.py -v
"""

import pytest
from pydantic import ValidationError

from agent_edit_document_v6 import (
    EDIT_DOCUMENT_V6_SYSTEM_PROMPT,
    EditDocumentAgentV6,
    EditPlan,
    InsertPatch,
    NormalizedPatch,
...
def test_replace_patch_rejects_end_line_zero():
    # v6's ReplacePatch is replace/delete only — no inverted ranges.
    with pytest.raises(ValidationError):
...
def test_edit_plan_rejects_empty_comment():
    with pytest.raises(ValidationError):
        EditPlan(reasoning="test plan", patches=[], status="done", comment="")


def test_edit_plan_rejects_empty_reasoning():
    # reasoning is the leading think-out-loud field; it must be present
    # and non-empty so the LLM actually does the warm-up step.
    with pytest.raises(ValidationError):
        EditPlan(reasoning="", patches=[], status="done", comment="ok")


def test_edit_plan_reasoning_is_first_field():
    # Field order matters for structured output: the model emits fields
    # in declaration order, so reasoning must come before patches.
    assert list(EditPlan.model_fields.keys())[0] == "reasoning"


def test_edit_plan_rejects_unknown_status():
    with pytest.raises(ValidationError):
        EditPlan(reasoning="test plan", patches=[], status="maybe", comment="x")  # pyright: ignore[reportArgumentType]
...
def test_system_prompt_mentions_both_ops():
    assert '"replace"' in EDIT_DOCUMENT_V6_SYSTEM_PROMPT
    assert '"insert"' in EDIT_DOCUMENT_V6_SYSTEM_PROMPT


def test_system_prompt_mentions_eof_marker():
    assert "EOF" in EDIT_DOCUMENT_V6_SYSTEM_PROMPT


def test_system_prompt_documents_status_and_comment():
    assert "status" in EDIT_DOCUMENT_V6_SYSTEM_PROMPT
    assert '"done"' in EDIT_DOCUMENT_V6_SYSTEM_PROMPT
    assert '"partial"' in EDIT_DOCUMENT_V6_SYSTEM_PROMPT
    assert '"unclear"' in EDIT_DOCUMENT_V6_SYSTEM_PROMPT
    assert "comment" in EDIT_DOCUMENT_V6_SYSTEM_PROMPT


# ----- handle() / agent wiring ------------------------------------------------

import db
from agent_config import EDIT_DOCUMENT_V6_UUID
...
def _stub_agent(app_ctx, monkeypatch, response_plan: EditPlan):
    agent = EditDocumentAgentV6(
        agent_uuid=EDIT_DOCUMENT_V6_UUID, name="edit_document_v6", send=lambda _: None
    )
...
def test_handle_returns_normalized_replace(app_ctx, monkeypatch):
    ...
    # Journal patches use the canonical (no-op-field) shape; the result echoes
    # the plan's leading `reasoning` field.
    assert result == {
        "ok": True,
        "reasoning": "test plan",
        "status": "done",
...
def test_handle_uses_logical_line_count_for_validation(app_ctx, monkeypatch):
    ...
    # is out of range. (Raw split sees 2 entries; v6 sees 1.)
    plan = EditPlan(
        reasoning="test plan",
    ...
    def stub(user_prompt, validator=None):
        try:
            ...
            raise RuntimeError(
                f"agent edit_document_v6: all models failed; last error: {e}"
            ) from e
