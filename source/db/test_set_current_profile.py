...
def test_deleting_current_profile_clears_pointer_atomically(app_ctx):
    """Deleting the active profile must clear profile.current and advance its
    change stamp in the SAME transaction — never leave a dangling uuid that
    silently disables every declared-profile block."""
