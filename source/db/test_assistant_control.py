...
def test_create_control_is_pending(run):
    c = db.create_assistant_control(run_id=run.id, command="stop")
    assert c.state == "pending"
    assert c.command == "stop"


def test_list_pending_controls_in_order(run):
    a = db.create_assistant_control(run_id=run.id, command="redirect",
                                    payload={"instruction": "focus on git"})
    b = db.create_assistant_control(run_id=run.id, command="stop")
    pending = db.list_pending_controls(run.id)
...
def test_mark_control_applied_excludes_it_from_pending(run):
    ...
    assert db.list_pending_controls(run.id) == []
