...
def test_cron_room_and_sender_seeded(app_ctx):
    room = db.session.query(Chatroom).filter_by(uuid=CRON_ROOM_UUID).first()
    assert room is not None and room.name == "cron"

    sender = db.session.query(ChatUser).filter_by(uuid=CRON_SYSTEM_UUID).first()
    assert sender is not None and sender.user_type == "agent"

    # The cron sender is a member of the cron room.
    assert db.session.query(ChatroomMember).filter_by(
...
def test_seed_chat_defaults_idempotent(app_ctx):
    ...
    rooms_before = db.session.query(Chatroom).filter_by(uuid=CRON_ROOM_UUID).count()
    msgs_before = db.session.query(ChatMessage).filter_by(room_uuid=CRON_ROOM_UUID).count()
    db.seed_chat_defaults()
    assert db.session.query(Chatroom).filter_by(uuid=CRON_ROOM_UUID).count() == rooms_before == 1
    assert db.session.query(ChatMessage).filter_by(room_uuid=CRON_ROOM_UUID).count() == msgs_before
...
def test_post_cron_event(app_ctx):
    ...
    try:
        ...
        assert db.session.query(ChatMessage).filter_by(uuid=msg.uuid).first() is not None
    finally:
        if msg is not None:
            db.session.execute(sa.delete(ChatMessage).where(ChatMessage.uuid == msg.uuid))
            db.session.commit()
