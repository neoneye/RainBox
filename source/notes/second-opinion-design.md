...
1. `<current_user_request>` — the user message the program is judged
   ...
   `<current_user_request_summary_markdown>` section that follows it and why
   a shortened request is never itself a ground to reject)
...
5. `<user_settings_json>` / `<user_profile>` — who is asking
