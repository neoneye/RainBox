...
- **One bulk PUT for placement and settings, dedicated endpoints for create and delete.** Dragging is inherently a whole-list operation, so batching it is right; but a shape that can express a deletion makes a truncated payload indistinguishable from a deliberate act, so it doesn't get to (`docs/ui-tree-persistence.md`). Per-node *update* endpoints (and per-parent `position` scoping) remain the refinement target if concurrent editors ever become real.
