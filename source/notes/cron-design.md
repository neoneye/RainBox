...
- **`PUT /cron/api/tree`** — the bulk save (debounced 250 ms on the page) of placement, order, names and each job's schedule + action settings. It **only updates rows that already exist**: matched rows update in place (preserving `created_at`; SQLAlchemy dirty-checking means unchanged rows emit no `UPDATE`), and a payload that omits an existing row — or names one the DB doesn't have — is a **400** that mutates nothing. It can neither create nor delete, so there is no `deletes` counter (`notes/ui-tree-persistence.md`). One guard:
  - **`version`** (optimistic concurrency) — the token from GET, derived from the *user-managed* fields only (scheduler bookkeeping — `next_run_at`/`last_fired_at`/`updated_at` — is excluded, so background firing never invalidates an open page). Missing → 400; stale → **409** + the current token, before any mutation. On 409 the page re-hydrates, resets a dangling selection, and shows a toast instead of clobbering the other writer. **Every** mutating endpoint returns the new token — the PUT, both creates, both duplicates and both deletes — so the client never holds a stale one. The page serializes its debounced PUTs so it can't 409 against its own previous save, and flushes a pending PUT before any create/delete so the two responses can't race; a failed initial hydrate leaves the token null, so a PUT of the resulting empty state is refused rather than wiping the real tree.
- **`POST /cron/api/folders`** `{name, parentId}` → `{ok, folder, version}`.
- **`POST /cron/api/jobs`** — the New-job builder's complete spec → `{ok, job, version}`. Settings run through the same `validate_cron_job_fields` the tree save uses, so a job can never be created in a shape a later save would reject.
- **`POST /cron/api/folders/<uuid>/duplicate`** — deep-copy the folder, its nested folders and every job inside them, with fresh uuids, right after the source. The top copy is `"<name> (copy)"` and **disabled** (which disables the whole copied subtree via inherited-enabled), so nothing fires before the operator has looked at it.
- **`POST /cron/api/jobs/<uuid>/duplicate`** — `"<name> (copy)"` right after the source, **disabled**, with no run history.
- **`DELETE /cron/api/folders/<uuid>`** — cascades the nested folders, their jobs and those jobs' run history → `{ok, version}`.
- **`DELETE /cron/api/jobs/<uuid>`** — the job and its whole `cron_run` history → `{ok, version}`.
...
Per-node **update** endpoints (`PATCH /cron/api/jobs/<uuid>`) remain the documented long-term refinement — the bulk PUT has proven sufficient for a single-operator app, and every placement/settings edit on the page already funnels through one debounced save. Creation and deletion are no longer part of that question: they have their own endpoints, because a shape that can express "delete these" turns a routine frontend bug into data loss.

### Validation

`db.validate_cron_tree` runs inside `cron_save_tree` — so *every* writer (the endpoint today, future MCP/agent editors) is covered — and raises `CronTreeError` **before any DB mutation**; the endpoint maps it to 400. Checks: payload shape (lists of objects); uuid shape + **global uniqueness across folders and jobs** (a node is identified globally by uuid — `/cron?id=<uuid>` — so a cross-kind collision would make the deep link ambiguous; uuids are normalized so case/format variants collide); reference integrity (parents/folders must exist in the same payload); **acyclic** folder nesting; and the per-job settings.

Those per-job settings checks live in `validate_cron_job_fields`, shared with `cron_create_job` so a job cannot enter the DB through creation in a shape the tree save would reject: `type` ∈ the action types; a message `target` must be empty or a uuid (so a stale room *name* can't be saved); `timezone` ∈ {`localtime`, `UTC`}; `maxRetries` an int 0–10; cron expression **empty (one-shot) or exactly 5 fields** of UI-grammar characters (`[0-9*/,-]`) — a shape check; croniter is the real parser.

Beyond validation, the save also refuses a payload that **omits** an existing folder or job, or that names one the DB doesn't hold — absence is a bug, not an instruction to delete.

## Frontend

The page shell + CSS live in `webapp/cron_views.py`; all logic in `static/cron.js`. State is two browser arrays (`cronFolders`/`cronRowsState`) hydrated from `GET /cron/api/tree`; placement and settings edits save back whole (debounced), while creation, duplication and deletion are immediate requests to their own endpoints. Layout matches `/chat` and `/modelgroups`: full-height split, independently scrolling panes. The tree follows the app-wide left-panel conventions (`notes/ui-left-panel-tree.md`): nested `<ul>`s with a left-border guide line, tinted + **bold** selection (exactly one node highlighted at a time, folder and job alike), outlined action buttons.
...
- **One bulk PUT for placement and settings, dedicated endpoints for create and delete.** Dragging is inherently a whole-list operation, so batching it is right; but a shape that can express a deletion makes a truncated payload indistinguishable from a deliberate act, so it doesn't get to (`notes/ui-tree-persistence.md`). Per-node *update* endpoints (and per-parent `position` scoping) remain the refinement target if concurrent editors ever become real.
